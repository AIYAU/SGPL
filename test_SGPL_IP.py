...
def test(im_width, im_height, channels):
    ...
    feature_encoder.load_state_dict(torch.load(f'./model/IP/IP_feature_encoder_16way_1shot.pkl'))
    print("load feature encoder success")
    Prototype_network.load_state_dict(torch.load(f'./model/IP/IP_network_16way_1shot.pkl'))
    ...
    AA = aa_temp
    f = open(f"result/IP/IP"  + '_' + str(np.sum(np.trace(matrix)) / float(n) * 100) + ".txt", 'w')   
