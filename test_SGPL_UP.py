...
def rn_predict(support_images, test_images, num):
    ...
    new_one_hot_labels = torch.zeros((9, 55), device=one_hot_labels.device)
...
def test(im_width, im_height, channels):
    ...
    feature_encoder.load_state_dict(torch.load(f'./model/UP/UP_feature_encoder_9way_1shot.pkl'))
    print("load feature encoder success")
    Prototype_network.load_state_dict(torch.load(f'./model/UP/UP_network_9way_1shot.pkl'))
    ...
    AA = aa_temp
    f = open(f"result/UP/UP"  + '_' + str(np.sum(np.trace(matrix)) / float(n) * 100) + ".txt", 'w')   
